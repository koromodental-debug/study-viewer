...
def classify_subject(title, search_text, subjects):
    ...
    if best_match and best_score >= 2:
        # 科目名を正規化して返す
        return normalize_subject(best_match['subject_name']), best_match['category']
...
def copy_files(html_files, qa_files):
    ...
    # HTMLをコピー（変更があるファイルのみ）
    html_mapping = {}
    copied = 0
    total = len(html_files)
    for i, (key, info) in enumerate(html_files.items(), 1):
        print(f"\r    HTML: {i}/{total}", end='', flush=True)
        dest_name = safe_filename(info['filename'])
        dest_path = DEST_HTML_DIR / dest_name
        src_path = Path(info['fullpath'])
        # 更新日時が異なる場合のみコピー
        if not dest_path.exists() or src_path.stat().st_mtime > dest_path.stat().st_mtime:
            shutil.copy(src_path, dest_path)
            copied += 1
        html_mapping[key] = f"html/{dest_name}"
    print(f"\r    HTML: {copied}/{total} files copied")

    # Q&Aをコピー（変更があるファイルのみ）
    qa_mapping = {}
    copied = 0
    total = len(qa_files)
    for i, (key, info) in enumerate(qa_files.items(), 1):
        print(f"\r    Q&A: {i}/{total}", end='', flush=True)
        dest_name = safe_filename(info['filename'])
    ...
    print(f"\r    Q&A: {copied}/{total} files copied")
...
def build_subject_index(html_files, qa_files, subjects):
    ...
    for key, html_info in html_files.items():
        ...
        # ファイル名ベースのソートキー（01_章_キーワード.html の形式）
        sort_key = html_info.get('filename', '')
...
def main():
    ...
    # 除外科目をフィルタ
    all_items = [item for item in all_items if item.get('subject') not in SUBJECT_EXCLUDE]

    # ソート（subjectコンテンツはsortKey=ファイル名順、legacyはtitle順）
    all_items.sort(key=lambda x: (
        x.get('source', 'legacy'),  # subjectを先に
        x.get('subjectCategory', 'その他'),
        x.get('subject', 'その他'),
        x.get('sortKey', '') or x['title']  # sortKeyがあればそれを使用
    ))
